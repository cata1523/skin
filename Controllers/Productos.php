...
class Productos extends Controller
{
    ...
    public function registrar()
    {
        if (isset($_POST['nombre']) && isset($_POST['precio'])) {
            $nombre = $_POST['nombre'];
            $precio = $_POST['precio'];
            $precioproducto = $_POST['preciopublico'];
            ...
            if (empty($nombre) || empty($precio) || empty($precioproducto)|| empty($cantidad))  {
                ...
                if (empty($id)) {
                    $data = $this->model->registrar($nombre, $descripcion, $precio, $precioproducto , $cantidad, $destino, $categoria);
                    if ($data > 0) {
                        if (!empty($imagen['name'])) {
                    ...
                    $data = $this->model->modificar($nombre, $descripcion, $precio, $precioproducto , $cantidad, $destino, $categoria, $id);
                    if ($data == 1) {
                        if (!empty($imagen['name'])) {
                            move_uploaded_file($tmp_name, $destino);
                        }
